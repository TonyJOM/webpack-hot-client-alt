...
// Webpack 4
ParserHelpers = ParserHelpers || require('webpack/lib/ParserHelpers');

const HotClientError = require('./HotClientError');

function addEntry(entry, compilerName, options) {
  const clientEntry = [`webpack-hot-client-alt/client?${compilerName || uuid()}`];
  let newEntry = {};
...
function validateEntry(entry) {
  ...
  if (type !== 'function') {
    if (!isArray && type !== 'object') {
      throw new TypeError(
        'webpack-hot-client-alt: The value of `entry` must be an Array, Object, or Function. Please check your webpack config.'
      );
    }
    ...
    if (!isArray && type === 'object') {
      for (const key of Object.keys(entry)) {
        ...
        if (!Array.isArray(value)) {
          throw new TypeError(
            'webpack-hot-client-alt: `entry` Object values must be an Array or Function. Please check your webpack config.'
          );
        }
      }
    }
